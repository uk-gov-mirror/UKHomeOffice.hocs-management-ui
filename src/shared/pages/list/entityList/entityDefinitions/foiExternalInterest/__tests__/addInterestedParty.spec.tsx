...
const AddInterestedParty = AddEntity(foiInterestedParty);

const renderComponent = () => render(
    <MemoryRouter>
        <AddInterestedParty history={history} location={location} match={match}></AddInterestedParty>
    </MemoryRouter>
);
...
describe('when the foiInterestedParty addEntity component is mounted', () => {
    ...
    describe('and the data is filled in', () => {
        ...
        describe('and the service call fails', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: 'Something went wrong while adding the interested party. Please try again.', title: GENERAL_ERROR_TITLE });
            });
        ...
        describe('and the service call fails with a 409', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: 'An interested party with those details already exists', title: VALIDATION_ERROR_TITLE });
            });
        });
    });
    describe('and the data is not filled in', () => {
        ...
        it('should set the error state', () => {
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(1,
                { key: 'title', value: 'The Interested party name is required' });
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(2,
                { key: 'simpleName', value: 'The Interested party code is required' });
        });
    });
});
