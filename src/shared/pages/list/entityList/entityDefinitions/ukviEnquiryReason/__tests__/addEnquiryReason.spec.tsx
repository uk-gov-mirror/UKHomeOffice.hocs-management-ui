...
import enquiryReason from '../ukviEnquiryReason';
...
const AddEnquiryReason = AddEntity(enquiryReason);
...
describe('when the ukviEnquiryReason addEntity component is mounted', () => {
...
describe('when the name is entered', () => {
    it('should be persisted in the page state', async () => {
        ...
        const nameElement = await waitForElement(async () => {
            return await wrapper.findByLabelText('Enquiry Reason name');
        });
...
describe('when the code is entered', () => {
    it('should be persisted in the page state', async () => {
        ...
        const codeElement = await waitForElement(async () => {
            return await wrapper.findByLabelText('Enquiry Reason code');
        });
...
describe('when the submit button is clicked', () => {
    describe('and the data is filled in', () => {
        ...
        describe('and the service call fails', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: 'Something went wrong while adding the enquiry reason. Please try again.', title: GENERAL_ERROR_TITLE });
            });
        ...
        describe('and the service call fails with a 409', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: 'Enquiry reason with that name already exists', title: VALIDATION_ERROR_TITLE });
            });
        });
    });
    describe('and the data is not filled in', () => {
        ...
        it('should set the error state', () => {
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(1, { key: 'title', value: 'The Enquiry Reason name is required' });
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(2, { key: 'simpleName', value: 'The Enquiry Reason code is required' });
        });
    });
});
