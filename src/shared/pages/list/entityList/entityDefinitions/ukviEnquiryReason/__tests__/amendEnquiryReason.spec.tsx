...
import ukviEnquiryReason from '../ukviEnquiryReason';
...
const AmendEnquiryReason = AmendEntity(ukviEnquiryReason);
...
describe('when the ukviEnquiryReason amendEntity component is mounted', () => {
    ...
    it('should display an error if the call to retrieve item details fails', async () => {
        ...
        await waitFor(() => {
            expect(setMessageSpy).toBeCalledWith({ title: GENERAL_ERROR_TITLE,
                description: 'There was an error retrieving enquiry reasons. Please try refreshing the page.' });
        });
...
describe('when the new name is entered', () => {
    it('should be persisted in the page state', async () => {
        ...
        fireEvent.change(nameElement, { target: { name: 'title', value: 'newTestEnquiryReasonTitle' } });

        await waitFor(() => {
            expect(reducerDispatch).toHaveBeenCalledWith({ type: 'SetTitle', payload: 'newTestEnquiryReasonTitle' });
        });
    });
});
...
describe('when the submit button is clicked', () => {
    ...
    describe('and the data is not filled in', () => {
        ...
        it('should set the error state', () => {
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(1, { key: 'title', value: 'The New enquiry reason name is required' });
        });
    });
});
...
describe('when the submit button is clicked', () => {
    describe('and the data is filled in', () => {
        ...
        describe('and the service call fails', () => {
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: 'Something went wrong while amending the enquiry reason. Please try again.', title: GENERAL_ERROR_TITLE });
            });
