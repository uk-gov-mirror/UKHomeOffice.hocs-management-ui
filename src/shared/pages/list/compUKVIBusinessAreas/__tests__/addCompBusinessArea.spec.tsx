...
const renderComponent = () => render(
    <MemoryRouter>
        <AddCompBusinessArea history={history} location={location} match={match}></AddCompBusinessArea>
    </MemoryRouter>
);
...
describe('when the submit button is clicked', () => {
    describe('and the data is filled in', () => {
        ...
        describe('and the service call fails', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: ADD_BUS_UNIT_ERROR_DESCRIPTION, title: GENERAL_ERROR_TITLE });
            });
        ...
        describe('and the service call fails with a 409', () => {
            ...
            it('should set the error state', () => {
                expect(setMessageSpy).toHaveBeenCalledWith({ description: DUPLICATE_BUS_UNIT_DESCRIPTION, title: VALIDATION_ERROR_TITLE });
            });
        });
    });
    describe('and the data is not filled in', () => {
        ...
        it('should set the error state', () => {
            expect(addFormErrorSpy).toHaveBeenNthCalledWith(1, { key: 'title', value: 'The Business Unit is required' });
        });
    });
});
